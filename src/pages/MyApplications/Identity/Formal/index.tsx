...
import FormalCheckModal from './components/FormalCheckModal'
import type { IdentityFormalDataType } from '@pages/MyApplications/index.d'
...
const Formal = () => {
   ...
   * 查看
   */
  const onCheck = (item: IdentityFormalDataType) => {
    console.log(item)
    setActiveItem(item)
    setOpen(true)
  }
   ...
   * 撤销
   */
  const onRevoke = (item: IdentityFormalDataType) => {
    Modal.confirm({
      title: '谨慎操作',
  ...
  const columns: ColumnsType<IdentityFormalDataType> = [
    {
      title: '应用名称',
  ...
  const data: IdentityFormalDataType[] = [
    {
      key: '1',
  ...
  return (
    <>
      ...
      <FormalCheckModal
        item={activeItem as IdentityFormalDataType}
        open={open}
        setOpen={setOpen}
      />
    </>
