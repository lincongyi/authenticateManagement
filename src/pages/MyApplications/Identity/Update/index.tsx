...
import type { IdentityUpdateDataType } from '@pages/MyApplications/index.d'
...
const Update = () => {
   ...
   * 查看
   */
  const onCheck = (item: IdentityUpdateDataType) => {
    console.log(item)
    setActiveItem(item)
    setOpen(true)
  }
   ...
   * 撤销
   */
  const onRevoke = (item: IdentityUpdateDataType) => {
    Modal.confirm({
      title: '谨慎操作',
  ...
  const columns: ColumnsType<IdentityUpdateDataType> = [
    {
      title: '应用名称',
  ...
  const data: IdentityUpdateDataType[] = [
    {
      key: '1',
  ...
  return (
    <>
      ...
      <UpdateCheckModal
        item={activeItem as IdentityUpdateDataType}
        open={open}
        setOpen={setOpen}
      />
    </>
