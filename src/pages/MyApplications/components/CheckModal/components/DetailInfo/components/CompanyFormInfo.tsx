...
const CompanyFormInfo = ({
  ...
  const onCompare = (key: keyof TCompanyInfo, title?: string) => {
    return before[key] === after[key] ? (
      <Typography.Text>{title || (after[key] as string)}</Typography.Text>
    ) : (
      <Typography.Text type='danger'>
        {title || (after[key] as string)}
      </Typography.Text>
    )
  }
  ...
  return (
    <>
          ...
          {after.certificatePhoto ? (
            <div
              className={`${style['img-wrap']} ${
                before.certificatePhoto !== after.certificatePhoto &&
                style.different
              }`}
            >
              <Image
      ...
      <Descriptions title='单位管理员信息' bordered column={2}>
