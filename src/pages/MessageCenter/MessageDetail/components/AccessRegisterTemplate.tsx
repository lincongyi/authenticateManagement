...
const AccessRegisterTemplate = ({ info }: { info: TGetMsgDetail }) => {
  const content = JSON.parse(info.content) as TApplyRecord['info']

  const items = [
    {
      label: '单位名称',
      children: content.companyName
    },
    {
      label: '单位简称',
      children: content.companyShortName
    },
    {
      label: '统一社会信用代码',
      children: content.certificateNum
    },
    {
      label: '信用代码证书',
      children: (
        <Image
          width={23}
          src={`data:image/png;base64,${content.certificatePhoto}`}
